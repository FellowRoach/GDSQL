...
### 用 SQLParser 执行 SQL 字符串

将任意 SQL 字符串解析为可执行的 `BaseDao` 对象，直接调用 `query()` 即可：
...
# 插入
GDSQL.SQLParser.parse_to_dao("INSERT INTO GameConfig.c_hero (id, name, hp) VALUES (101, '法师', 80)").query()
...
SQL 关键字大小写不敏感。解析器内置 LRU 缓存（1024 条），重复查询更快。

### 使用 QueryResult

`QueryResult` 封装查询结果，提供状态检查和数据访问方法：
...
if result and result.ok():
    var head = result.get_head()   # 列名：["id", "name", "hp"]
    var data = result.get_data()   # 数据行：[[1, "Warrior", 200], ...]
    print("行数：", result.size())
    
    for row in data:
        var name = row[head.find("name")]
        print(name)
else:
    print("查询失败：", result.get_err() if result else "null")
```

### 跨库查询

使用 `数据库.表名` 格式引用其他数据库中的表：
...
- **条件查询**：WHERE、AND、OR、IN、NOT IN、Godot 内置操作符或函数（例如contains）
- **排序与分组**：ORDER BY、GROUP BY
- **分页**：LIMIT、OFFSET
- **连接查询**：LEFT JOIN（支持链式多表连接；支持跨数据库联表查询!）
- **联合查询**：UNION ALL
...
<p align="center">
  <img src="https://raw.githubusercontent.com/jinyangcruise/GDSQL-ICON/refs/heads/main/Screenshot-2026-06-17-193925.jpg" alt="GDSQL 截图 1" width="800"/>
</p>
<p align="center">
  <img src="https://raw.githubusercontent.com/jinyangcruise/GDSQL-ICON/refs/heads/main/Screenshot-2026-06-17-193934.jpg" alt="GDSQL 截图 2" width="800"/>
</p>
<p align="center">
  <img src="https://raw.githubusercontent.com/jinyangcruise/GDSQL-ICON/refs/heads/main/Screenshot-2026-06-17-193947.jpg" alt="GDSQL 截图 3" width="800"/>
</p>
<p align="center">
  <img src="https://raw.githubusercontent.com/jinyangcruise/GDSQL-ICON/refs/heads/main/Screenshot-2026-06-17-194000.jpg" alt="GDSQL 截图 4" width="800"/>
</p>
<p align="center">
  <img src="https://raw.githubusercontent.com/jinyangcruise/GDSQL-ICON/refs/heads/main/Screenshot-2026-06-17-194147.jpg" alt="GDSQL 截图 5" width="800"/>
</p>
