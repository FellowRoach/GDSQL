...
func use_conf_db() -> GDSQL.BaseDao:
	var game_conf_db_name = GDSQL.get_setting_game_conf_db_name()
	if game_conf_db_name == "":
		return _assert_false("use_conf_db", "Game conf db name is not set!")
	var game_conf_db_dir = GDSQL.RootConfig.get_database_data_path(game_conf_db_name)
	if game_conf_db_dir == "":
		return _assert_false("use_conf_db", "Game conf db dir is empty!")
				...
				for col in columns_def:
					...
					if col["NN"]:
						...
						if __data[col_name] == null:
							result._affected_rows = 0
							result._err = "field '%s' is implicitly cast to null which is not support by ConfigFile." % col_name
							push_error(result.get_err())
							result._cost_time = Time.get_unix_time_from_system() - begin_time
							return result
...
## 将复杂表达式转为简单表达式
func _simplify_expression(
		expression: String,
		sql_input_names: Dictionary = { },
		sql_static_inputs: Array = [],
		sql_varying_inputs: Dictionary = { },
):
	var possible_sql = GDSQL.SQLParser.replace_nested_sql_expression(
		expression,
		sql_input_names,
		sql_static_inputs,
		sql_varying_inputs,
...
func ___loop_table_row(
		result: Array,
		all_datas: Dictionary,
		loop_tables: Array,
		loop_index: int,
		curr_row: Dictionary,
		head: Array,
		table_definations: Dictionary,
) -> bool:
	if loop_index == loop_tables.size():
		...
		if all_datas[table].size() > 0:
			...
			for row in all_datas[table]:
				...
				var check_result = condition_wrapper.cond(
					simple_expression[0],
					input_names,
					simple_expression[1],
				).check(__inputs, acc_row)
...
func _get_init_datas(
		db_name: String,
		table_name: String,
		table_alias: String,
		password,
		fill_primary_key: String,
		cond: String,
		all_table_defination: Dictionary,
		all_datas: Dictionary,
		curr_dependency: Dictionary = { },
):
	if all_datas.has(table_alias):
		return

	var indexed_names = all_table_defination[table_alias].filter(func(v): return v.Index).map(
		func(v):
				...
				if not dealed:
					...
					if not __lack_table.is_empty():
						if __collect_lack_table_enabled:
							return null
						return _assert_false("computing field", "Unknown table(s): %s" % ", ".join(__lack_table))
					var value = GDSQL.GDSQLUtils.evalute_command_with_agg(
						agg_func_obj,
						simple_expression[0],
						[],
						[],
			...
			# 数据集为空，还是要继续检查，因为数据集为空不能说明是否用了聚合函数
				...
				for i in real_select.size():
					...
					if total_agg_func_obj.has(i):
						...
						GDSQL.AggregateFunctions.prepare_done(agg_func_obj.id)
						var simple_expression = _simplify_expression(
							real_select[i].name_4_computing,
							__final_input_names,
							__inputs,
							data,
						)
			...
			for alias in all_datas:
				if alias == __table_alias:
					real_select.append_array(
						__get_table_columns(__db_name, __table, __table_alias).map(fill_select_name.bind(alias)),
					)
				else:
					var a_left_join = __left_join.get_left_join_by_alias(alias)
					real_select.append_array(
						__get_table_columns(a_left_join.get_db(), a_left_join.get_table(), alias).map(fill_select_name.bind(alias)),
					)
