...
func gen_select_node() -> GraphNode:
	...
	# 根据选择的数据库来更新表名备选项
	schema_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	table_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	fields_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	where_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	groupby_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	order_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	limit_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	graph_node.node_enable_status.connect(
		func(enabled):
...
func gen_left_join_node() -> GraphNode:
	...
	cond_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
			graph_node.push_redraw_slot_control(3, 2)
			match prop:
				"On":
...
func gen_table_node(columns: Array, table_datas: Array, is_union_all: bool, join_conds: Array, v_scroll_h: int = 0, old_graph_node: GraphNode = null) -> GraphNode:
	...
	for j in columns.size():
		...
		if dealed_columns.has(real_column_name):
			...
			new_column_prop_name.push_back(
				{
					"type": j,
					"prop": col_name,
					"col_name": columns[j]["Column Name"],
					"table_path": columns[j]["db_path"].path_join(columns[j]["table_name"]),
				},
			) # 记录j列数据的属性名称等信息
			...
			if columns[j]["is_field"]:
				hint[col_name] = {
					"hint": columns[j]["Hint"],
					"hint_string": columns[j]["Hint String"],
					"type": columns[j]["Data Type"],
				}
				# 记录键位置信息
	...
	if table.editable:
		...
		var update_btn_disable_status = func(_prop, _new_val, _old_val):
			if not table:
				EditorInterface.get_editor_toaster().push_toast(
					"Please re-select your data in the table.",
		...
		btn_apply.pressed.connect(
			func():
				...
				for i in deleted_datas:
					...
					for table_path: String in grouped_modified_data:
						...
						if grouped_modified_data[table_path]["PK_key"] == null:
							...
							base_dao.where(
								"%s == %s" % [
									grouped_modified_data[table_path]["PK_key"],
				...
				btn_close_refresh.disabled = get_from_nodes(graph_node, "Select").filter(
					func(v):
						return v.enabled
...
func gen_insert_node() -> GraphNode:
	...
	var fields_dict_obj = GDSQL.DictionaryObject.new(
		{ "ColumnName": false },
		{ "ColumnName": { "usage": PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_READ_ONLY } },
	)
...
func gen_sql_node() -> GraphNode:
	...
	code_editor.text_changed.connect(
		func():
...
func add_link_node(
		...
		left_offset = 0,
		left_limit = 1000,
		left_show_column_name = false,
		left_show_column_value = true,
		left_font_size = 14,
		left_processor = "",
		right_db = "",
		right_password = "",
		...
		right_offset = 0,
		right_limit = 1000,
		right_show_column_name = false,
		right_show_column_value = true,
		right_font_size = 14,
		right_processor = "",
		asize = null,
		pos_offset = null,
...
func gen_link_node() -> GraphNode:
	...
	left_schema_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	right_schema_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
	...
	left_table_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
			graph_node.push_redraw_slot_control(6, 2)
			match prop:
				"Table1":
					...
					if new_val != "" and mgr.databases[GDSQL.RootConfig.validate_name(left_schema_dict_obj._get("Schema1"))]["tables"].has(GDSQL.RootConfig.validate_name(new_val)):
						...
						left_link_prop_dict_obj.reset_hint(
							{
								"LinkColumnName": {
	...
	right_table_dict_obj.value_changed.connect(
		func(prop, new_val, _old_val):
				...
				"Table2":
					...
					if new_val != "" and mgr.databases[GDSQL.RootConfig.validate_name(right_schema_dict_obj._get("Schema2"))]["tables"].has(GDSQL.RootConfig.validate_name(new_val)):
						...
						for col in mgr.databases[GDSQL.RootConfig.validate_name(right_schema_dict_obj._get("Schema2"))]["tables"][GDSQL.RootConfig.validate_name(new_val)]["columns"]:
							...
							adata[col["Column Name"]] = true
							hints[col["Column Name"]] = { "hint": PROPERTY_HINT_NONE, "hint_string": "", "type": TYPE_BOOL }
						right_link_prop_dict_obj.reset_hint(
							{
								"LinkColumnName": {
									"hint": PROPERTY_HINT_ENUM,
									"hint_string": ",".join(adata.keys()),
	...
	left_code_edit.text_changed.connect(
		func():
			left_other_options._set("processor", left_code_edit.text)
	)
	left_other_options.set_custom_display_control(
		"processor",
		left_code_edit,
		func(new_val):
	...
	right_code_edit.text_changed.connect(
		func():
			right_other_options._set("processor", right_code_edit.text)
	)
	right_other_options.set_custom_display_control(
		"processor",
		right_code_edit,
		func(new_val):
			...
			func():
				...
				if log_history:
					mgr.add_log_history.emit(
						...
						begin_time,
						action,
						"%d row(s) returned" % (ret.get_data().size()),
						ret.get_cost_time(),
					) # 去掉表头
		...
		func():
			...
			mgr.request_user_enter_password.emit(
				...
				func():
					...
					var left_select = left_column_dict_obj.get_data().keys().map(
						func(v):
					...
					mgr.request_user_enter_password.emit(
						...
						func():
							...
							var right_select = right_column_dict_obj.get_data().keys().map(
								func(v):
							...
							for row: Array in left_datas:
								...
								btn_cancel_all.pressed.connect(
									func():
										for detail_panel in grid.get_children():
								...
								btn_apply.pressed.connect(
									func():
										...
										btn_close_refresh.disabled = get_from_nodes(node, "Link").filter(
											func(v):
												return v.enabled
											...
											func(custom_action):
												if custom_action == "close_and_refresh":
													...
													if btn_apply.disabled:
														...
														mgr.create_confirmation_dialog(
															"You have some modifications that have not been executed.\n" \
...
func get_selected_nodes():
	return graph_edit.get_children().filter(
		func(v):
...
func _load_nodes(nodes: Dictionary, connections: Array, pos_offset: Vector2, auto_name: bool, select_all = false):
	...
	for node_name in nodes:
		...
		match type:
			...
			"Result":
				node = await add_table_node(
					params["columns"],
					params["table_datas"],
					params["is_union_all"],
					params["join_conds"],
					params.get("v_scroll_height", 0),
					asize,
					position_offset,
					a_name,
				)
			"Insert":
				node = await add_insert_node(
					params["Schema"],
					params["_password"],
					params["Table"],
					params["Fields"],
			...
			"Link":
				node = await add_link_node(
					params["link_db"],
					params["link_password"],
					params["link_table"],
					params["right_columns"],
					params["link_left_col"],
					params["link_right_col"],
					params["left_db"],
					params["left_password"],
					params["left_table"],
					params["left_link_col"],
					params["left_cols_shown"],
					params["left_where"],
					params["left_order_by"],
					params["left_offset"],
					params["left_limit"],
					params["left_show_column_name"],
					params["left_show_column_value"],
					params["left_font_size"],
					params["left_processor"],
					params["right_db"],
					params["right_password"],
					params["right_table"],
					params["right_link_col"],
					params["right_cols_shown"],
					params["right_where"],
					params["right_order_by"],
					params["right_offset"],
					params["right_limit"],
					params["right_show_column_name"],
					params["right_show_column_value"],
					params["right_font_size"],
					params["right_processor"],
					asize,
					position_offset,
					a_name,
					false,
