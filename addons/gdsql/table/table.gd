...
func _ready() -> void:
	...
	v_scroll_bar.visibility_changed.connect(
		func():
	...
	for i in fake_columns.size():
		...
		if i == 0:
			...
			button.mouse_entered.connect(
				DisplayServer.cursor_set_custom_image.bind(
					load("res://addons/gdsql/img/arrow_down.svg"),
					DisplayServer.CURSOR_HELP,
					Vector2(12, 12),
				),
			)
			c.dragger_visibility = HSplitContainer.DRAGGER_HIDDEN_COLLAPSED
	...
	for i in data.size():
		...
		if show_frame and i == 1:
			...
			control.mouse_entered.connect(
				DisplayServer.cursor_set_custom_image.bind(
					load("res://addons/gdsql/img/arrow_right.svg"),
					DisplayServer.CURSOR_HELP,
					Vector2(12, 12),
				),
			)
	...
	for row in range(start_pos.x, end_pos.x):
		for col in range(start_pos.y, end_pos.y):
			...
			if selected_borders.is_empty():
				if row == start_pos.x:
					if sb == null:
						sb = DEFAULT_BORDER_STYLE.duplicate()
					sb.border_width_top = 2
				if col == start_pos.y:
					if sb == null:
						sb = DEFAULT_BORDER_STYLE.duplicate()
			...
			if diff.x > -diff.y:
				...
				if pos_row > start.x:
					...
					if rect.has_point(panel_container.get_parent().get_local_mouse_position()):
						...
						add_autofill_border(
							start,
							Vector2(pos_row, end.y) + Vector2.ONE,
			...
			if -diff.x > diff.y:
				...
				if pos_col > start.y:
					...
					if rect.has_point(panel_container.get_parent().get_local_mouse_position()):
						...
						add_autofill_border(
							start,
							Vector2(end.x, pos_col) + Vector2.ONE,
...
func _on_v_box_container_mouse_entered():
	if editable:
		DisplayServer.cursor_set_custom_image(
			load("res://addons/gdsql/img/movetoolarrows.svg"),
			DisplayServer.CURSOR_FORBIDDEN,
			Vector2(12, 12),
		)
